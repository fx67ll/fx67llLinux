...
chown -R $NGINX_USER:$NGINX_USER /usr/share/nginx/html-82 || { echo "设置文件所有者失败，尝试使用单用户"; chown -R $NGINX_USER /usr/share/nginx/html-82 || { echo "仍然无法设置文件所有者，请检查用户权限"; exit 1; } }
chmod -R 755 /usr/share/nginx/html-82 || { echo "设置文件权限失败"; exit 1; }
echo "已设置正确的文件权限"
```
