...
echo "本次编译项目名称：$PROJECT_NAME"
echo "项目端口：$PROJECT_PORT，混沌项目端口：$PROJECT_CHAOSHEN_PORT"
...
if [ -f "./dist.tar.gz" ]; then
  ...
  echo "⚠️警告: dist.tar.gz文件不存在，继续执行"
fi

# 创建新的tar包
tar -zcvf dist.tar.gz ./* || { echo "⚠️警告: 创建项目tar包失败"; exit 1; }
echo "✅ 成功创建项目tar包"

# 切换到项目Nginx目录
cd /usr/share/nginx/html-$PROJECT_PORT || { echo "⚠️警告: 无法进入项目Nginx目录"; exit 1; }
...
# 复制tar包到项目Nginx目录
cd /root/.jenkins/workspace/$PROJECT_NAME/$PROJECT_FILE_PATH || { echo "⚠️警告: 无法进入项目工作目录"; exit 1; }
scp /root/.jenkins/workspace/$PROJECT_NAME/$PROJECT_FILE_PATH/dist.tar.gz /usr/share/nginx/html-$PROJECT_PORT || { echo "复制项目tar包失败"; exit 1; }
rm -f ./dist.tar.gz
echo "✅ 已复制项目tar包并删除源文件"

# 解压tar包到项目Nginx目录
cd /usr/share/nginx/html-$PROJECT_PORT || { echo "⚠️警告: 无法进入项目Nginx目录"; exit 1; }
tar -zxvf dist.tar.gz -C ./ || { echo "⚠️警告: 解压项目tar包失败"; exit 1; }
rm -f dist.tar.gz

# 设置项目正确的文件权限
chown -R $NGINX_USER:$NGINX_USER /usr/share/nginx/html-$PROJECT_PORT || { echo "⚠️警告: 设置项目文件所有者失败，尝试使用单用户"; chown -R $NGINX_USER /usr/share/nginx/html-$PROJECT_PORT || { echo "⚠️警告: 仍然无法设置项目文件所有者，请检查用户权限"; exit 1; } }
chmod -R 755 /usr/share/nginx/html-$PROJECT_PORT || { echo "⚠️警告: 设置项目文件权限失败"; exit 1; }
echo "✅ 已设置项目正确的文件权限"

# 检查项目index.html是否存在
if [ -f "/usr/share/nginx/html-$PROJECT_PORT/index.html" ]; then
  echo "✅ 项目部署成功: index.html文件存在"
else
  echo "⚠️警告: 项目index.html文件不存在，请检查项目结构"
fi
